 ...
 */
public class ordenadoresNeo {

    neoOrdenador ordFirst;
    neoOrdenadorSecond ordSecond;

    ordenadoresNeo(String path, int registerSize) throws Exception {
        this.ordFirst = new neoOrdenador(path, registerSize);
        this.ordSecond = new neoOrdenadorSecond(path, registerSize);
    }

    public void sortFirst() throws Exception {
        ordFirst.sort();
    }

    public void sortSecond() throws Exception {
        ordSecond.sort();
    }

    class neoOrdenadorSecond {
         ...
         * @throws Exception
         */
        public neoOrdenadorSecond(String path, int registerSize) throws Exception {
            if (!path.endsWith(".db"))
        ...
        private void __createArchives() throws Exception {
        ...
        private int distribute() throws Exception {
         ...
         * @throws Exception
         */
        private int interpolate() throws Exception {
            int numberOfBlocks = 0,
                    i = 0;
         ...
         * @throws Exception
         */
        private void readRegistersAndWriteOrdered(Binario arc) throws Exception {
            Boolean[] restrictions = new Boolean[this.originalFiles.length];
            for (int i = 0; i < restrictions.length; i++)
                restrictions[i] = false;

            Pokedex[] prev = new Pokedex[this.originalFiles.length];
            ...
            while (!this.__blockWasReaded(restrictions)) {
                ...
                for (int i = 0; i < this.originalFiles.length; i++) {
                    if (!this.originalFiles[i].isEOF() && !restrictions[i]) {
                        Pokedex obj = this.originalFiles[i].read();

                        if (prev[i] == null)
                            prev[i] = obj;

                        if (prev[i].getID() <= obj.getID()) {
                            if (min == null) {
                                min = obj;
                                positionOfMinObj = i;
    ...
    class neoOrdenador {
         ...
         * @throws Exception
         */
        public neoOrdenador(String path, int registerSize) throws Exception {
            if (!path.endsWith(".db"))
                throw new IllegalArgumentException("The file must have the extension \".db\".");
